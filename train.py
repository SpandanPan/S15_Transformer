...
def run_validation(model, validation_ds,tokenizer_src,tokenizer_tgt,max_len,device,print_msg,global_step):
  ...
  with torch.no_grad():
    ...
    if writer:
      ...
      writer.add_scaler('validation cer', cer, gloabl_step)
      writer.flush()

      metric = torchmetrics.WordErrorRate()
      wer = metric(predicted,expected)
      writer.add_scaler('validation wer', wer, gloabl_step)
      writer.flush()


      metric = torchmetrics.BLEUScore()
      bleu = metric(predicted,expected)
      writer.add_scaler('validation bleu', bleu, gloabl_step)
      writer.flush()
...
for epoch in range(initial_epoch,config['num_epochs']):
  ...
  for batch in batch_iterator:
    ...
    batch.iterator.set_postfix({"loss":f"{loss.item():6.3f}"})

    writer.add_scaler('train loss',loss.item(),gloabl_step)
    writer.flush()
